...
static void Rotate(FAVLTREENODE **P, FAVLTREENODE *p, int d) {
    FAVLTREENODE **CP, *cp, **GCP, *gcp; // temporary copies of child + grandchild of P for swapping
    if(d==-1) {
...
void FAvlTreeInsert(FAVLTREE *tree, foint key, foint info)
{
    FAVLTREENODE *p = tree->root, **P = &(tree->root), // P is a locative used to trace the path
	*a,**A, *b,**B, *c,**C, *r,**R; // temporary node pointers and their locatives
    ...
    p = (FAVLTREENODE*) Calloc(1,sizeof(FAVLTREENODE)); // insert a new leaf
...
Boolean FAvlTreeLookDel(FAVLTREE *tree, foint key, foint *pInfo)
{
    FAVLTREENODE *p = tree->root, **P = &(tree->root);
    while(p)
    {
...
static int FAvlTreeTraverseHelper (foint globals, FAVLTREENODE *p, pFointTraverseFcn f)
{
    int cont = 1;
    if(p) {
	if(p->left) cont = FAvlTreeTraverseHelper(globals, p->left, f);
	if(cont) cont = f(globals, p->key, p->info);
	if(cont && p->right) cont = FAvlTreeTraverseHelper(globals, p->right, f);
    }
    return cont;
}

int FAvlTreeTraverse (foint globals, FAVLTREE *tree, pFointTraverseFcn f)
{
    return FAvlTreeTraverseHelper(globals, tree->root, f);
}

static int _avlTreeSanityNodeCount, _avlTreeSanityPhysicalNodeCount;
static Boolean FAvlTreeSanityHelper ( FAVLTREENODE *p, pCmpFcn cmpKey )
{
    if(p) {
	++_avlTreeSanityPhysicalNodeCount;
	if(p->left) { assert(cmpKey(p->left->key, p->key)<0); FAvlTreeSanityHelper(p->left, cmpKey); }
	++_avlTreeSanityNodeCount;
	if(p->right) { assert(cmpKey(p->key, p->right->key)<0); FAvlTreeSanityHelper(p->right, cmpKey); }
    }
    return true;
}

Boolean FAvlTreeSanityCheck ( FAVLTREE *tree)
{
    _avlTreeSanityNodeCount = 0;
    _avlTreeSanityPhysicalNodeCount = 0;
    assert(0 <= tree->n);
    FAvlTreeSanityHelper(tree->root, tree->cmpKey);
...
*/
foint FAvlTreeLookupKey(FAVLTREE *tree, foint info)
{
    FAVLTREENODE *p = tree->root;
    if(p)
    {
	...
	{
	    foint key = FAvlTreeLookupKey(p->left, info);
	    if(key)
		return key;
	    else
		return FAvlTreeLookupKey(p->right, info);
	}
    }
    return NULL;
}
...
static void FAvlTreeFreeHelper(FAVLTREE *tree, FAVLTREENODE *t)
{
    if(t)
    {
	FAvlTreeFreeHelper(tree, t->left);
	FAvlTreeFreeHelper(tree, t->right);
	tree->freeKey(t->key);
...
void FAvlTreeFree(FAVLTREE *tree)
{
    FAvlTreeFreeHelper(tree, tree->root);
    assert(tree->n == 0);
    free(tree);
}
